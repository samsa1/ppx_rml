...
The OCaml compiler provides a technology called PPX. A PPX rewriter is preprocess that modifies the OCaml code just after it was parsed. This allow to generate complex code automatically just like macros in other programming languages.

To use a PPX rewriter, the users can add the following line into their `dune` file (e.g. for the `ppx_rml` PPX rewriter):
...
associated subtree is given to the handler function associated with the flag. This handler function takes an AST node as argument and returns a new AST node (in practive, the types involved are a bit more complicated).
...
Extensions are written with `%`. Contrarily to attributes, extensions are nodes of the AST. During preprocessing, the AST node is rewritten by the handler function associated with the corresponding extension.
