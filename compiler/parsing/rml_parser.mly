...
simple_core_type2:
    QUOTE ident
      { mkte(RmlPtype_var $2) $loc }
  | type_longident
      { mkte(RmlPtype_constr($1, [])) $loc }
  | simple_core_type2 type_longident
      { mkte(RmlPtype_constr($2, [$1])) $loc }
  | LPAREN core_type_comma_list RPAREN type_longident
      { mkte(RmlPtype_constr($4, List.rev $2)) $loc }
  | simple_core_type PROCESS
      { mkte(RmlPtype_process ($1, Def_static.Dontknow)) $loc }
  | simple_core_type PROCESS PLUS
      { mkte(RmlPtype_process ($1, Def_static.Noninstantaneous)) $loc }
  | simple_core_type PROCESS MINUS
      { mkte(RmlPtype_process ($1, Def_static.Instantaneous)) $loc }
;
simple_core_type_or_tuple:
    simple_core_type                            { $1 }
  | simple_core_type STAR core_type_list
      { mkte(RmlPtype_tuple($1 :: List.rev $3)) $loc }
;
