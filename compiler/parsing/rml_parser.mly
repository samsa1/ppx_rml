...
let mkexpr_until body cfg_when_opt_expr_opt_list =
...
let _ghte d = { pte_desc = d; pte_loc = symbol_gloc () };;
...
structure_item:
    LET rec_flag let_bindings
      { match $3 with
          [{ppatt_desc = Ppatt_any; _}, exp] -> mkimpl(Pimpl_expr exp)
...
simple_core_type2:
    QUOTE ident
      { mkte(RmlPtype_var $2) }
  | type_longident
      { mkte(RmlPtype_constr($1, [])) }
  | simple_core_type2 type_longident
      { mkte(RmlPtype_constr($2, [$1])) }
  | LPAREN core_type_comma_list RPAREN type_longident
      { mkte(RmlPtype_constr($4, List.rev $2)) }
  | simple_core_type PROCESS
      { mkte(RmlPtype_process ($1, Def_static.Dontknow)) }
  | simple_core_type PROCESS PLUS
      { mkte(RmlPtype_process ($1, Def_static.Noninstantaneous)) }
  | simple_core_type PROCESS MINUS
      { mkte(RmlPtype_process ($1, Def_static.Instantaneous)) }
;
simple_core_type_or_tuple:
    simple_core_type                            { $1 }
  | simple_core_type STAR core_type_list
      { mkte(RmlPtype_tuple($1 :: List.rev $3)) }
;
