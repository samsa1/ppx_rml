...
(* instanciation *)
let instance { ts_desc = ty; ts_binders = _; ts_rbinders = _ } =
  let ty_i = copy ty in
  cleanup ();
  ty_i
...
let rec unify expected_ty actual_ty =
  if expected_ty == actual_ty then ()
	...
	Type_var, _ ->
      ...
      | Type_constr
	  ({ info = Some { constr_abbr=Constr_abbrev(params,body) }; gi = _ }, args),
	_ ->
...
let rec filter_arrow ty =
  ...
  | Type_constr({info=Some{constr_abbr=Constr_abbrev(params,body)};gi=_},args) ->
