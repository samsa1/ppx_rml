...
let rec print_reactivity ff k =
  ...
  | React_raw (k1, { react_desc = React_var; react_index = _; react_level = _ }) ->
      fprintf ff "(%a + ..)"
        print_reactivity k1
  | React_raw (k1, k2) ->
      fprintf ff "(%a + %a)"
...
let print ff ty =
  ...
  print ff 0 ty;
  pp_print_flush ff ()
let print_scheme ff { ts_desc = ty; ts_binders = _; ts_rbinders = _; } = print ff ty
...
let print_type_declaration ff gl =
  ...
  let { type_kind = td; type_arity = ta; type_constr = _ } = Global.info gl in
  pp_open_box ff 2;
