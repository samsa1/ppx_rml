	  ...
	  in
          fun () ->
            ...
            ref_f := Some f_await_top;
						let () = print_int (List.length w_list) in
						let () = print_endline " <- Length of w_list" in
            List.iter
              (fun w ->
                w := (step_wake_up w) :: !w;
		toWakeUp := w :: !toWakeUp)
...
(**************************************************)
	...
	try
	  sched ();
		print_string "[";
		List.iter (fun i -> print_int (List.length !i); print_string " ") !toWakeUp;
		print_endline "] to wake up";
	  eoi := true;
	  wakeUp weoi;
	  wakeUpAll ();
	  sched ();
